...
@Contract(LongSpan.class)
public class LongSpanContractTest<T extends LongSpan> {

	private IProducer<T> producer;
	private LongSpan span;

	public LongSpanContractTest() {
	}
	...
	public void testStartLengthEndAgreement() {
		Assert.assertEquals("End value is incorrect", LongSpan.calcEnd(span), span.getEnd());
		Assert.assertEquals("Length value is incorrect", LongSpan.calcLength(span), span.getLength());
	}
	...
	public void testSubspanOverlaps() {
		final LongSpan span2 = LongSpan.fromEnd(span.getOffset() + 1, span.getEnd() - 1);
	...
	public void testOverlapLeadingEdge() {
		final long mid = span.getOffset() + (span.getLength() / 2);
		final LongSpan span2 = LongSpan.fromLength(span.getOffset() - 1, mid);
